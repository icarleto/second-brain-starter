...
Anything you are making: writing, video, software, ventures. The output, not the job that funds it.
...
FROM "3. Projects"
WHERE contains(area, "Creation")
SORT priority ASC
```

## Open tasks

Tasks tagged `#creation`.

```dataview
TASK
FROM "1. Journal" OR "3. Projects"
WHERE !completed AND contains(lower(text), "#creation")
SORT file.name DESC
```
